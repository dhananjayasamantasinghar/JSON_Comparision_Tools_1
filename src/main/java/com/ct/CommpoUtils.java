...
public class CommpoUtils {
	...
	public static void doSimpleMapping(POD_Employee_1 podEmployee, POA_Employee poa_Employee, Field field,
			String mappingValue) {
		try {
	...
	public static void doComplexMapping(POD_Employee_1 podEmployee, POA_Employee poa_Employee, Field field,
			String mappingValue) throws InstantiationException, IllegalAccessException, ClassNotFoundException,
	...
	private static void setFinalData(POD_Employee_1 podEmployee, Field field, String[] values, int i, String value)
	...
	private static void setFirstComplexType(POD_Employee_1 podEmployee, POA_Employee poa_Employee, Field field,
			...
			if (field2.getName().equalsIgnoreCase(newValue)) {
				...
				for (Method method : declaredMethods) {
					if (method.getName().equalsIgnoreCase("set" + values[values.length - 1])) {
						field.setAccessible(true);
						method.invoke(obj, ReflectionUtils.getField(field, podEmployee));
						Method[] declaredMethods2 = poa_Employee.getClass().getDeclaredMethods();
						for (Method method2 : declaredMethods2) {
							if (method2.getName().equalsIgnoreCase("set" + newValue)) {
								method2.invoke(poa_Employee, obj);
								flag = true;
								objectFactory.put(value, obj);
							}
						}
	...
	public static POA_Employee convertToPoaEmployee(POD_Employee_1 podEmployee)
			throws InstantiationException, IllegalAccessException, ClassNotFoundException, NoSuchMethodException,
			SecurityException, IllegalArgumentException, InvocationTargetException {
		POA_Employee poa_Employee = new POA_Employee();
